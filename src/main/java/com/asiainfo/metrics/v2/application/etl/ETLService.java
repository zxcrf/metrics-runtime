...
import com.asiainfo.metrics.common.infrastructure.minio.KpiComputeService.KpiDataRecord;
import com.asiainfo.metrics.common.infrastructure.repository.KpiDataSourceRepository;
import com.asiainfo.metrics.common.infrastructure.repository.KpiMetadataRepository;
import com.asiainfo.metrics.common.model.domain.KpiDefinition;
import com.asiainfo.metrics.common.model.domain.KpiModel;
import com.asiainfo.metrics.v2.api.dto.ModelExecutionResult;
import com.asiainfo.metrics.v2.api.dto.SrcTableCompleteResponse;
import com.asiainfo.metrics.v2.application.webhook.WebhookNotificationService;
import com.asiainfo.metrics.v2.infrastructure.repository.ETLLogRepository;
import com.asiainfo.metrics.v2.infrastructure.repository.ModelDependencyRepository;
import com.asiainfo.metrics.v2.infrastructure.repository.TaskLogRepository;
import com.asiainfo.metrics.v2.infrastructure.repository.TaskLogRepository.TaskLog;
...
public class ETLService {
    ...
    /**
     * 旧版 processETL 方法（保持向后兼容）
     * 直接调用模型执行，不走表到达流程
     */
    public ETLResult processETL(String tableName, String opTime) {
        log.info("[ETL] Legacy processETL: table={}, opTime={}", tableName, opTime);
        ...
        try {
            ...
            if (!computeResult.success()) {
                log.error("[ETL] Compute failed: {}", computeResult.message());
                return ETLResult.error("指标计算失败: " + computeResult.message());
            }

            List<KpiDataRecord> data = computeResult.data();
            log.info("[ETL] Compute succeeded, {} records", data.size());

            // 2. 写入 Parquet 并上传到 MinIO
            ParquetWriter.WriteResult writeResult = parquetWriter.writeToParquet(data);

            if (!writeResult.success()) {
                log.error("[ETL] Write failed: {}", writeResult.message());
                return ETLResult.error("Parquet 写入失败: " + writeResult.message());
            }

            log.info("[ETL] Write succeeded, {} stored", writeResult.storedCount());
