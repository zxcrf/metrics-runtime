...
public class ParquetWriter {
     ...
     * @return 写入结果
     */
    public WriteResult writeToParquet(List<KpiDataRecord> records) {
        try {
            if (records.isEmpty()) {
            ...
            Map<String, List<KpiDataRecord>> kpiGroup = records.stream()
                    .collect(Collectors.groupingBy(KpiDataRecord::kpiId));

            int totalStored = 0;

            // 为每个 KPI 创建单独的 Parquet 文件
            for (Map.Entry<String, List<KpiDataRecord>> entry : kpiGroup.entrySet()) {
                String kpiId = entry.getKey();
                List<KpiDataRecord> kpiRecords = entry.getValue();
                ...
                // 获取第一条记录用于提取元信息
                KpiDataRecord firstRecord = kpiRecords.get(0);
